...
void main(void) 
{
    ...
    while(1)
    {
        ...
        adres_map = map(adres_aux);     // Almacenar valor mapeado a 8 bits
    }
}
...
void config_io(void)
{
    ...
    ANSELH = 0b00000001;        // Pin analógico        (AN8 = RB2)
    
    TRISAbits.TRISA5 = 1;       // Entrada Slave Select (SPI - Slave)
    TRISBbits.TRISB2 = 1;       // Entrada Pot          (ADC)
...
void config_adc(void)           // Configuración del ADC
{
    ...
    ADCON0bits.CHS = 8;         // Selección del canal 8
...
uint16_t map(uint16_t num)      // Mapeado de ADRES
{
    uint16_t res;               // Definir variable de salida
    res = num/4;                // Factor de mapeo 255:1023 = 1:4
    return res;
}